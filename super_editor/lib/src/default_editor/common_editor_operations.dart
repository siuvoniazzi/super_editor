...
class CommonEditorOperations {
  ...
  bool convertParagraphByPatternMatching(String nodeId) {
    ...
    final taskItemMatch = RegExp(r'^\[]\s$');
    final hasTaskItemMatch = taskItemMatch.hasMatch(textBeforeCaret);

    _log.log('_convertParagraphIfDesired',
        ' - text before caret: "$textBeforeCaret"');
    if (hasUnorderedListItemMatch ||
        hasOrderedListItemMatch ||
        hasTaskItemMatch) {
      _log.log(
      ...
      final newNode = hasUnorderedListItemMatch
          ...
          : hasOrderedListItemMatch
              ...
              : TaskItemNode.open(id: node.id, text: adjustedText);
  ...
  bool insertBlockLevelNewline() {
    ...
    } else if (extentNode is TaskItemNode) {
      if (extentNode.text.text.isEmpty) {
        // The list item is empty. Convert it to a paragraph.
        return convertToParagraph();
      }
  ...
  /// Converts the [TextNode] with the current [DocumentComposer] selection
  /// extent to a [TaskItemNode] of the given [type], or does nothing if the
  /// current node is not a [TextNode], or if the current selection spans
  /// more than one node.
  ///
  /// Returns [true] if the selected node was converted to a [TaskItemNode],
  /// or [false] if it wasn't.
  bool convertToTaskItem(TaskItemType type, AttributedText text) {
    if (composer.selection == null) {
      return false;
    }
    if (composer.selection!.base.nodeId != composer.selection!.extent.nodeId) {
    ...
    final nodeIndex = editor.document.getNodeIndex(node);
    final newNode = TaskItemNode(id: nodeId, itemType: type, text: text);
